...
// Departments needed for core slot dropdowns
...
export default function Planner() {
  ...
  // Load all core-eligible courses once for dropdowns
  useEffect(() => {
  ...
  // Load completed + prior courses
  useEffect(() => {
    if (!user) { setCompletedSet(new Set()); setPriorCourses([]); return }
    ...
    <div>
      ...
      {viewMode === 'my-plan' && (
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button onClick={() => setShowResetConfirm(true)} className="text-xs text-gray-500 border border-gray-300 rounded-lg px-3 py-1.5 hover:bg-gray-50 transition-colors">Reset to suggested</button>
          </div>
        </div>
      )}
      ...
      <p className="text-sm text-gray-500 mb-6">
        {viewMode === 'suggested'
          ...
          : <>Build your personal plan. Drag courses and core slots between semesters. Use ✕ to remove, or assign a specific course to any core slot.</>
        }
